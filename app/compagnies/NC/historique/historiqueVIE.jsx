...
export default function DetailNCVIE() {
    const router = useRouter()
    const handleClick = (email,societe,date) => {
        console.log(`You clicked me! ${email}`);
        router.push('/compagnies/NC/historique/voir?email='+email.toString()+'&societe='+societe.toString()+'&date='+date.toString());
      };
    ...
    const columns = [
{
        ...
        },
        {
            ...
            cell: (d) => [
               ...
               <button  key={d.date_soumission+d.annee} onClick={() => handleClick(d.email_membre, d.nom_societe, d.date_soumission)}className='mx-3'>
            ...
            //     </svg>
            //   </button>,
            <button  key={d.date_soumission+d.annee} onClick={() => handleClick(d.email_membre, d.nom_societe, d.date_soumission)} type="button"> <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            const result = await getNCVIEComp(); // Mettre getNCVIE
...
//                 setIsClient(true)
//                 if(filtre == "annee"){
//                     setfilteredItems(users.filter(item => item.annee && item.annee.toLowerCase().includes(filterText.toLowerCase())))
//                 }
//                 if(filtre == "date_soumission"){
//                     setfilteredItems(users.filter(item => item.date_soumission && item.date_soumission.toLowerCase().includes(filterText.toLowerCase())))
//                 }
//                 if(filtre == "trimestre"){
//                     setfilteredItems(users.filter(item => item.trimestre && item.trimestre.toLowerCase().includes(filterText.toLowerCase())))
//                 }
...
//                 }
//                 if(filtre == "statut"){
//                     setfilteredItems(users.filter(item => item.statut && item.statut.toLowerCase().includes(filterText.toLowerCase())))
//                 }
