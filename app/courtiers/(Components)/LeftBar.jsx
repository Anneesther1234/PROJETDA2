...
function LeftBar() {
    ...
    useLayoutEffect(() => {
        ...
        if (path[1] === "courtiers" && window.location.pathname.length === 2) {
            setMenu("accueilC");
        } else {
            switch (path[2]) {
                case "NC":
    ...
    return (
        <div className="w-[15%] bg-orange h-screen p-3 flex-col sticky top-0">
            <div className='w-full h-[93%]'>
                <Link href='/courtiers' className="flex space-x-1 items-center" onClick={() => setMenu('accueilC')}>
                ...
                <div className="flex-col pt-3 space-y-2">
                    <Link href='/courtiers' className="text-orangetxt hover:text-white" onClick={() => setMenu('accueilC')}>MENU</Link>
                    <hr className="border-orangetxt"/>
                    <ul>
                        <li>
                            <Link href="/courtiers/NC" className={clsx('flex space-x-5 items-center hover:bg-orangebg rounded-md p-1', {' bg-orangebg ':menu == "nc"})} onClick={() => setMenu('nc')}>
                        ...
                        <li className="mt-2">
                            <Link href="#" className={clsx('flex space-x-5 items-center hover:bg-orangebg rounded-md p-1', {' bg-orangebg p-1':menu == "dossierA"})} onClick={() => setMenu('dossierA')}>
                        ...
                        <li className="mt-2">
                            <Link href="/courtiers/profils" className={ menu == "profils" ? "flex space-x-5 items-center bg-orangebg p-1 rounded-md" : "flex space-x-5 items-center hover:bg-orangebg hover:rounded-md hover:p-1"} onClick={() => setMenu('profils')}>
