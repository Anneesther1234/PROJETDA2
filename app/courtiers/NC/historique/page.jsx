...
import  getNcCOURTIERComp from '../../../../lib/api/NC/COURTIER/getNcCOURTIERComp'
...
export default function DetailNCCOURTIER() {
    const router = useRouter()
    const handleClick = (email,societe,date) => {
        console.log(`You clicked me! ${email}`);
        router.push('/courtiers/NC/historique/voircourtier?email='+email.toString()+'&societe='+societe.toString()+'&date='+date.toString());
    };
   
    const columns = [
{
            name: 'Date soumission',
            selector: row => row.dateSoumission,
            sortable: true,
        },
        ...
        },
        {
            ...
            cell: (d) => [
               ...
               <button key={d.email_membrel} onClick={() => handleClick(d.email_membre, d.societe, d.dateSoumission)} className='mx-3'>
  ...
  <path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" />
            ...
            //     </svg>
            //   </button>,
            <button  key={d.email} type="button"> <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
    ...
    const [users,setUsers] = useState([{dateSoumission:"",annee:"",trimestre:"", societe:"",type_assurance:"",statut:"",action:""}])
    ...
    useEffect(() => {
        console.log("update");
         ...
         const fetchData = async () => {
            const result = await getNcCOURTIERComp(); // Mettre getNCVIE
        ...
        if (users !== null) {
            setIsClient(true);
            setfilteredItems(
                users ? users.filter(item => item.dateSoumission && item.dateSoumission.toLowerCase().includes(filterText.toLowerCase()), item => item.annee && item.annee.toLowerCase().includes(filterText.toLowerCase()), item => item.statut && item.statut.toLowerCase().includes(filterText.toLowerCase())):[]

            );
          } else {
            setfilteredItems([{ dateSoumission: "", annee: "", trimestre: "", societe: "", type_assurance: "", statut: "", action: "" }]);
          }
          ...
          intervalRef.current = setInterval(() => {
            ...
            if (users != null){
                setIsClient(true)
                console.log(users);
                setfilteredItems(users ? users.filter(item => item.dateSoumission && item.dateSoumission.toLowerCase().includes(filterText.toLowerCase())) : [])
    ...
    return ( 
        <>
           <div className='pt-3'>
            <h1 className='font-bold text-gris text-center text-2xl'>Liste des notes de conjecture COURTIER</h1>
