...
import getNCCOURTIERTemp from '../../../../../lib/api/NC/COURTIER/getNCCOURTIERTemp'
...
export default function DetailNCCOURTIER() {
    ...
    const handleClick = (email,societe,date) => {
        ...
        router.push('/employeDA/NC/historique/voircourtier?email='+email.toString()+'&societe='+societe.toString()+'&date='+date.toString());
    };
    ...
    const columns = [
{
        ...
        },
        {
            ...
            cell: (d) => [
               ...
               <button key={d.email_membre} onClick={() => handleClick(d.email_membre, d.societe, d.dateSoumission)}>
    ...
    useEffect(() => {
        console.log("update");
         ...
         const fetchData = async () => {
            const result = await getNCCOURTIERTemp(); // Mettre getNCVIE
        ...
        if (users !== null) {
            setIsClient(true);
            setfilteredItems(
                users ?   users.filter(item => item.societe && item.societe.toLowerCase().includes(filterText.toLowerCase())):[]
            );
          } else {
          ...
          intervalRef.current = setInterval(() => {
            ...
            if (users != null){
                setIsClient(true)
                console.log(users);
                setfilteredItems(users.filter(item => item.societe && item.societe.toLowerCase().includes(filterText.toLowerCase())))
             }else{
                setIsClient(false)
                 setfilteredItems([{dateSoumission:"",annee:"",trimestre:"", societe:"",type_assurance:"",statut:"",action:""}])
             }
