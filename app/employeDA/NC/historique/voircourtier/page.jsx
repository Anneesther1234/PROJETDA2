...
import Image from "next/image"
     ...
     const handleSubmit = () => {
      ...
      const validerNote = async (event) => {
        ...
        if(email && societe && date){
          const url = "http://localhost:9191/api/COURTIER/final/"
    ...
    return (
       ...
       {isDetail && <form onSubmit={validerNote} className="p-[2%] border border-vert shadow-md">  <table className="w-full whitespace-no-wrap">
        <tbody>
        <tr className="h-15.6  ">
          ...
          <tr className="h-15.6 border">
            <td colSpan="4" className="h-15.6 border px-4 py-2 border-r-1 border-solid ">
             <label htmlFor="">Total des Charges ou Frais Généraux</label> 
            </td>
            <td className="border-t-0">
            <div className="md:w-48 p-2" >
    ...
    <div className="w-[30%] bg-bleu rounded-lg ">
    <Link href={"/employeDA/NC/historique/voircourtier/refusercourtier?email="+email.toString()+"&societe="+societe.toString()+"&date="+date.toString()} > <button type="button" className="font-medium bg-red-500 hover:bg-red-700  rounded-lg w-full text-center px-5 py-3 text-white shadow-md text-md" >Refuser</button> </Link>    
    </div>
