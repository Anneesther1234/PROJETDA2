...
import Image from "next/image"
  ...
  const handleSubmit = () => {
    setResSubmit(true);
      ...
      const validerNote = async (event) => {
        ...
        if(email && societe && date){
          const url = "http://localhost:9191/api/IARD/final/"
    ...
    <div className="w-[30%] bg-bleu rounded-lg ">
    <Link href={"/employeDA/NC/historique/voiriard/refuseriard?email="+email.toString()+"&societe="+societe.toString()+"&date="+date.toString()} > <button type="button" className="font-medium bg-red-500 hover:bg-red-700  rounded-lg w-full text-center px-5 py-3 text-white shadow-md text-md" >Refuser</button> </Link>    
    </div>
