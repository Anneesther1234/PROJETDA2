...
export default async function getNCVIEComp(){
    try{
        if(cookies().has("token")){
            //  console.log(cookies().get("token")["value"].toString());
            const res = await fetch("http://localhost:9191/api/IARD/rechercheNoteParNomSociete", {
                method: 'GET',
                headers: { 
            ...
            if(res.ok){
                const NoteIARDComp = await res.json()
                return NoteIARDComp;
